...
CLASSIFICATION_ID_COLS = ["Name", "SMILES", "Label"]
...
def remove_high_nan_rows_classification(df: pd.DataFrame, threshold: float, id_cols):
    # NaN fraction is computed over descriptor columns only -- including
    # Name/SMILES/Label in the denominator would dilute the fraction and
    # under-count rows that are actually mostly-missing in their
    # descriptors.
...
def remove_high_nan_cols_classification(df: pd.DataFrame, threshold: float, id_cols):
    # Only descriptor columns are candidates for removal by NaN fraction;
    # id_cols (Name/SMILES/Label) are always preserved regardless of their
    # own (normally zero) NaN fraction.
    descriptor_cols = [c for c in df.columns if c not in id_cols]
    ...
    retained_cols = [c for c in df.columns if c in id_cols or c in keep_descriptor_cols]
...
def paired_preprocess_classification(train_df, test_df, y_test_df, target_column, compound_nan_threshold, descriptor_nan_threshold, method):
...
class Remove_high_nan_compounds_Classification:
    ...
    def run(self, descriptor_data_path, threshold):
        try:
            output_dir = os.path.join(folder_paths.get_output_directory(), "Classification", "04_Descriptor_Preprocessing")
            os.makedirs(output_dir, exist_ok=True)
            df = pd.read_csv(descriptor_data_path)
            initial_rows = len(df)
            df, removed_count = remove_high_nan_rows_classification(df, threshold, CLASSIFICATION_ID_COLS)
...
class Remove_high_nan_descriptors_Classification:
    ...
    def run(self, descriptor_data_path, threshold):
        try:
            ...
            df, removed_count = remove_high_nan_cols_classification(df, threshold, CLASSIFICATION_ID_COLS)
...
class Descriptor_preprocessing_Classification:
    ...
    def preprocess(self, descriptor_data_path, compounds_nan_threshold, descriptors_nan_threshold, imputation_method):
        try:
            ...
            df, inf_summary, inf_report_paths = audit_missing_and_infinite_values(df, output_dir, CLASSIFICATION_ID_COLS)
            df, compounds_removed = remove_high_nan_rows_classification(df, compounds_nan_threshold, CLASSIFICATION_ID_COLS)
            df, descriptors_removed = remove_high_nan_cols_classification(df, descriptors_nan_threshold, CLASSIFICATION_ID_COLS)
...
class Paired_Descriptor_Preprocessing_Classification:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    def run(self, training_data_path, holdout_data_path, holdout_targets_path, target_column,
        ...
        try:
            ...
            train_out, test_out, y_test_out, recipe, excluded_df = paired_preprocess_classification(
...
NODE_CLASS_MAPPINGS = {
    ...
    "Paired_Descriptor_Preprocessing_Classification": Paired_Descriptor_Preprocessing_Classification,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "Descriptor_preprocessing_Classification": "Descriptor Preprocessing (Whole-Dataset / No Split)",
    "Paired_Descriptor_Preprocessing_Classification": "4. Descriptor Preprocessing",
}
